...
products_df = pd.read_csv('ProductsClean_FN.csv')
...
def cosine_recommendation(df, cosine_similarities, product_id, num_recommendations=5, selected_brands=None):
    ...
    recommendations = df.iloc[similar_indices, :][['item_id', 'name', 'description', 'rating', 'list_price', 'brand']]
...
def search_cosine_recommendation(text, df, tfidf_vectorizer, tfidf_matrix, cosine_similarities, num_recommendations=5, selected_brands=None):
    ...
    recommendations = df.iloc[most_similar_indices][['item_id', 'name', 'description', 'rating', 'list_price', 'brand']]
...
if page == 'Product Name':
    st.title('Recommendation by Product Name or Description')
    ...
    sort_by = st.selectbox('Sort By', ['Rating', 'List Price', 'Brand'])
    ...
    if st.button('Get Recommendations'):
        ...
        elif sort_by == 'List Price':
            results = results.sort_values(by=['list_price'])
        elif sort_by == 'Brand':
            results = results.sort_values(by=['brand'])

        # Rename columns and format 'Product ID'
        results.columns = ['Product ID', 'Product Name', 'Product Description', 'Rating', 'List Price', 'Brand']
        results['Product ID'] = results['Product ID'].astype(str).str.replace(',', '')
...
if page == 'Product ID':
    ...
    if st.button('Get Recommendations'):
        ...
        # Rename columns and format 'Product ID'
        recommendations.columns = ['Product ID', 'Product Name', 'Product Description', 'Rating', 'List Price', 'Brand']
...
st.sidebar.info('This is a product recommendation app using machine learning model.')